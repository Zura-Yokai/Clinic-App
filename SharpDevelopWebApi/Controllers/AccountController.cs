...
namespace SharpDevelopWebApi.Controllers
{
    public class AccountController : ApiController
	{
        ...
        public IHttpActionResult RegisterUser(RegisterViewModels newUser) // You can add more parameter here ex LastName, FirstName etc
        {
...
//        	   return BadRequest("Username must be in a valid email format.");
            ...
            if (userId != null)
            {
            	...
            	if(newUser.role == "doctor" || newUser.role == "Doctor")
            	{
            		var doctor = new Doctor();
            		doctor.userId = userId.Value;            		
            		doctor.lastName = newUser.lastName;
            		doctor.firstName = newUser.firstName;
            		doctor.email = newUser.email;
            		doctor.address = newUser.address;
            		doctor.birthDate = newUser.birthDate;
            		doctor.gender = newUser.gender;
            		doctor.specialization = newUser.specialization;
            		doctor.phone = newUser.phone;
            		_db.Doctors.Add(doctor);
            		_db.SaveChanges();
            	}
            	else if(newUser.role == "patient" || newUser.role == "Patient")
            	{
            		var p = new Patient();
            		...
            		p.address = newUser.address;
            		p.birthDate = newUser.birthDate;
            		p.gender = newUser.gender;	
            		p.phone = newUser.phone;            		
            		_db.Patients.Add(p);
            		_db.SaveChanges();
            	}
